...
namespace Commons {
  namespace Utf8Convert {
    ...
    // Throws Utf8ConversionException on conversion errors
    // (e.g. invalid UTF-8 sequence found in input string).
    std::vector<std::wstring> Utf8ToUtf16(const std::vector<std::string>& utf8vector) {

      // Result of the conversion
      std::vector<std::wstring> utf16vector;

      //
      for(auto utf8 : utf8vector) {
        utf16vector.push_back(Utf8ToUtf16(utf8));
      }
    ...
    // Throws Utf8ConversionException on conversion errors
    // (e.g. invalid UTF-8 sequence found in input string).
	std::vector<std::string> Utf16ToUtf8(const std::vector<std::wstring>& utf16vector) {

		// Result of the conversion
		std::vector<std::string> utf8vector;

		//
		for (auto utf16 : utf16vector) {
			utf8vector.push_back(Utf16ToUtf8(utf16));
		}
